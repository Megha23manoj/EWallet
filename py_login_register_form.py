...
root = Tk()
connection = mysql.connector.connect(host='localhost', user='root', port='3306', password='admin123', database='EWALLET')
c = connection.cursor()
...
def bestUsers():
    ...
    for rowval in result:
            ...
            counter = counter+1
    query="SELECT stid,sendamount,date_initiated,sendidentifier,date_completed,txntype FROM creditDebitTxn where txntype='Debit' and sendamount=(SELECT MAX(sendamount) FROM creditDebitTxn where txntype='Debit')"
    c.execute(query)
    result=c.fetchall()
    header = ['STId','SendAmount','Date_Initiated','SendIdentifier','Date_Completed','TxnType']
    lc=0
    ...
    if(personId.isdigit()):
        ...
        if(len(userPhoneRes)== 0):
            ...
            if(len(phoneRes)==0):
                ...
                connection.commit()
                reqFromQuery = "INSERT INTO `FROM` (RTid, Identifier) VALUES (%s, %s)"
                c.execute(reqFromQuery,(reqId,personId))
                connection.commit()
